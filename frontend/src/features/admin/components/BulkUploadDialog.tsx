...
const TEMPLATE = `name,grade,parent_name,parent_phone,parent_phone2,parent_email,home_address,home_lat,home_lng,pickup_time,route_name
Asha Kamau,Grade 3,Jane Kamau,+254700111222,,jane@example.com,Kileleshwa,-1.2820,36.7780,06:45,Express 1 — Morning`;
...
export function BulkUploadDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const res = await api.post("/api/students/bulk", { students });
      setResult(res);
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Upload a CSV or Excel file. Required columns: name, grade, parent name, parent phone.
          </p>
